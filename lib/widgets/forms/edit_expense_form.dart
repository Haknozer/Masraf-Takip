  ...
  Future<void> _updateExpense(ExpenseModel expense, GroupModel group) async {
    ...
    if (_paymentType == PaymentType.fullPayment) {
      ...
      if ((paidTotal - amount).abs() > 0.01) {
        ErrorSnackBar.showWarning(
          context,
          'Ödenen tutarların toplamı ${amount.toStringAsFixed(2)} TL olmalı',
        );
        return;
      }
    ...
    try {
      ...
      if (_paymentType == PaymentType.fullPayment) {
        ...
        final entries = paidAmounts!.entries.toList()
